...
t = 0:dt:T_total;
N = length(t);
function [y, u_sig] = simulate_pid(ref_val, use_antiwindup, Kp, Ki, Kd, dt, N, U_max, U_min)
    ...
    u_sig = zeros(1, N);
    for k = 1:N
        r = ref_val; % Βηματική είσοδος
        e = r - x;        
        derivative = (e - prev_error) / dt;        
        if use_antiwindup
            u_temp = Kp*e + Ki*integral + Kd*derivative;
            if (u_temp >= U_max && e > 0) || (u_temp <= U_min && e < 0)
                % Μην αυξάνεις τον ολοκληρωτή (Clamping)
                integral = integral; 
            else
        ...
        u_calc = Kp*e + Ki*integral + Kd*derivative;
        if u_calc > U_max
            u = U_max;
        elseif u_calc < U_min
            u = U_min;
...
[y1, u1] = simulate_pid(1, false, Kp, Ki, Kd, dt, N, U_max, U_min);
[y2, u2] = simulate_pid(10, false, Kp, Ki, Kd, dt, N, U_max, U_min);
[y3, u3] = simulate_pid(10, true, Kp, Ki, Kd, dt, N, U_max, U_min);
figure;
subplot(2,1,1);
...
yline(1, 'k:');
title('Σύγκριση Αποκρίσεων (Κανονικοποιημένες στο 1)');
legend('Step=1 (Linear)', 'Step=10 (No Anti-windup)', 'Step=10 (With Anti-windup)');
grid on; ylabel('Κανονικοποιημένη Θέση');
subplot(2,1,2);
plot(t, u2, 'r--', 'LineWidth', 1.5); hold on;
plot(t, u3, 'b', 'LineWidth', 1.5);
yline(5, 'k--', 'Limit');
title('Σήμα Ελέγχου u(t) για Step=10');
legend('No Anti-windup', 'With Anti-windup');
grid on; ylabel('Είσοδος u'); ylim([-1 6]);
